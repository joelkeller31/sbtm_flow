...
class MeanFlowMatchingTrainer(Trainer):
    ...
    def sample_t_and_r_indices(self, size, large_interval_prob=0.3):
        bins = torch.linspace(5000, 0, steps=size+1)
        t = torch.zeros(size)
        r = torch.zeros(size)
    ...
    def get_train_loss(self, epoch: int, **kwargs) -> torch.Tensor:
        ...
        # Sample random times between r and t
        ep = torch.rand_like(tau_t)  # Random values in [0, 1]
        t_var = tau_r + ep * (tau_t - tau_r)  # Random times between r and t
        
        alpha = (t_var - tau_t) / (tau_r - tau_t + 1e-8)  # Normalize to [0,1]

        interpolated_samples = x_t * (1 - alpha) + x_r * alpha
        ...
        # Compute both the function value and its jvp
